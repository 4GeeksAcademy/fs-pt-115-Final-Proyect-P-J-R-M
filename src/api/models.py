...
class User(db.Model):
    ...
    sent_chats: Mapped[List["Chat"]] = relationship("Chat", foreign_keys="[Chat.user_one]", backref="sender")
    received_chats: Mapped[List["Chat"]] = relationship("Chat", foreign_keys="[Chat.user_two]", backref="receiver")
...
class Post(db.Model):
    ...
    def serialize(self):
        return {
            ...
            "day_exchange": self.day_exchange.strftime("%d/%m/%Y") if self.day_exchange else None,
